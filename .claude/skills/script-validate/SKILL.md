...
```bash
curl -X POST http://localhost:8080/api/tools/script-validate \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:8080/api/tools/script-validate -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
