...
```bash
curl -X POST http://localhost:8080/api/tools/node-duplicate \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:8080/api/tools/node-duplicate -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
