...
```bash
curl -X POST http://localhost:8080/api/tools/filesystem-list \
...
> Or pipe via stdin:
> ```bash
> curl -X POST http://localhost:8080/api/tools/filesystem-list -H "Content-Type: application/json" -d @- <<'EOF'
> {"param": "value"}
> EOF
> ```
